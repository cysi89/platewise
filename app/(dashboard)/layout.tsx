...
function buildWeeks(t: any): Week[] {
  return [0, 1, 2].map(offset => ({
    id: offset,
    weekOffset: offset,
    label: offset === 0 ? t("weekly.thisWeek") : offset === 1 ? t("weekly.nextWeek") : t("weekly.weekAfter"),
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const lang = localStorage.getItem("genie-language") || "en"
      const recipes = await fetchAllRecipes(lang)
    ...
    // Reload recipes when language changes
    const onLangChange = async (lang: string) => {
      const newRecipes = await fetchAllRecipes(lang)
      if (newRecipes.length > 0) {
        setMenus(newRecipes)
        // Update week selections to use new language recipe names
        setWeeks(prev => prev.map(week => ({
          ...week,
          label: week.weekOffset === 0 ? i18n.t("weekly.thisWeek") : week.weekOffset === 1 ? i18n.t("weekly.nextWeek") : i18n.t("weekly.weekAfter"),
          selections: week.selections.map(s => ({
            ...s,
            menu: s.menu ? newRecipes.find(m => m.id === s.menu!.id) || s.menu : null
          }))
        })))
      }
    }
...
function IngredientsTab({ weeks, userPrefs }: { weeks: Week[], userPrefs: UserPrefs }) {
  ...
  // Avg calories per planned day
  const plannedDays = selections.filter(s => s.menu)
  const avgCalories = plannedDays.length > 0
            ...
            {[
              ...
              <div key={card.label} style={{ background: "var(--white)", borderRadius: "var(--radius)", border: `1px solid ${card.color === "#dc2626" ? "#fecaca" : "var(--border)"}`, padding: "14px 18px", display: "flex", gap: 12, alignItems: "center" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, fontWeight: 600, color: "var(--text)", marginTop: 1 }}>{card.label}</div>
          ...
          <div className="grid-ingredients">
            {Object.entries(categories).map(([cat, items]) => (
              <div key={cat} style={{ background: "var(--white)", borderRadius: "var(--radius)", border: "1px solid var(--border)", overflow: "hidden" }}>
                ...
                {items.map(ing => {
                  const key = ing.name.toLowerCase()
                  const bought = checkedItems[key] || false
                  return (
                    <div key={ing.name} onClick={() => toggleItem(key)} style={{
                      padding: "10px 18px", borderBottom: "1px solid var(--border)",
                      display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8,
                      cursor: "pointer", transition: "background 0.15s",
                      background: bought ? "#f8f8f8" : "var(--white)",
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
                        ...
                        <div style={{
                          ...
                          border: bought ? "none" : "2px solid var(--border)",
                          background: bought ? "var(--green)" : "transparent",
                          display: "flex", alignItems: "center", justifyContent: "center",
                        ...
                        <div style={{ minWidth: 0 }}>
                          <span style={{
                            ...
                            color: bought ? "var(--text-muted)" : "var(--text)"
                          }}>{ing.name}</span>
                          <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 1 }}>{ing.days.join(", ")}</div>
                        </div>
                      </div>
                      <span style={{ background: bought ? "var(--cream-dark)" : "var(--green-pale)", borderRadius: 8, padding: "3px 10px", fontSize: 12, fontWeight: 600, flexShrink: 0, color: bought ? "var(--text-muted)" : "var(--green)" }}>{ing.amount} {ing.unit}</span>
                    </div>
                  )
                })}
              </div>
